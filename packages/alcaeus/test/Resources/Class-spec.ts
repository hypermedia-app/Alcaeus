...
import { ClassMixin } from 'alcaeus-model/Mixins/Class.js'
...
describe('Class', () => {
  ...
  beforeEach(() => {
    dataset = $rdf.dataset()
    hydraClassNode = $rdf.clownface({ dataset })
  ...
  describe('getting operations', () => {
    ...
    it('should combine own operations with inherited operations', async () => {
      ...
      const clas = new Class($rdf.clownface({ dataset }).namedNode(vocab.DraftIssue))
