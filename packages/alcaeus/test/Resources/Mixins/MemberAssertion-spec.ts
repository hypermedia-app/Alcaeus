...
import { MemberAssertionMixin } from 'alcaeus-model/Mixins/MemberAssertion.js'
...
describe('MemberAssertion', () => {
  ...
  beforeEach(() => {
    dataset = $rdf.dataset()
    node = $rdf.clownface({ dataset }).blankNode()

    memberAssertion = new MemberAssertion(node, $rdf)
  })
  ...
  describe('shouldApply', () => {
    it('should return true if resource is object of hydra:manages property', () => {
      ...
      // when
      const result = MemberAssertionMixin.shouldApply(memberAssertion as any)
    ...
    describe('matches', () => {
      ...
      describe('by subject and predicate type', () => {
        ...
        it('returns true if pattern is matching resource object and resource property', () => {
          ...
          // when
          const isMatch = memberAssertion.matches({
            predicate: $rdf.rdfine().createEntity(node.namedNode(foaf.knows)),
            subject: $rdf.rdfine().createEntity(node.namedNode('http://example.com/person/Tomasz')),
          })
