...
describe('FetchUtil', () => {
  ...
  describe('resource', () => {
    ...
    it('should fetch linked JSON-LD context', async () => {
      ...
      const context = {}
      mockFetch.returns(responseBuilder().body(context).build())
      ...
      expect(wrapper).to.have.property('jsonLdContext', context)
    })
  })
  ...
  describe('invokeOperation', () => {
    ...
    it('should set not set content-type header for FormData bodies', async () => {
      ...
      // then
      expect(mockFetch)
        .to.have.been.calledWith(sinon.match.any, sinon.match(value => {
          return value.headers.has('content-type')
        }))
    })
  })
})
